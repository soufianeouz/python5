...
class SensorStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        ...
        return (
            f"Sensor analysis: {total_items} readings processed, "
...
class StreamProcessor:
    ...
    def process_all(
        self, batches: List[List[Any]], criteria: str = "High-priority"
    ) -> None:
        ...
        for stream in self.streams:
            ...
            if isinstance(stream, SensorStream):
                print(
                    f"- Sensor data: {stats['readings processed']} "
                    "readings processed"
                )
            elif isinstance(stream, TransactionStream):
                print(
                    f"- Transaction data: {stats['operations processed']} "
                    "operations processed"
                )
            elif isinstance(stream, EventStream):
                print(
                    f"- Event data: {stats['events processed']} "
                    "events processed"
                )
            i += 1
        ...
        for item in self.streams:
            if isinstance(item, SensorStream):
                string += (
                    f"{len(item.filter_data(batches[i], criteria))} "
